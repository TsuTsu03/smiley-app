...
export default function AdminOverview({ onNav }: { onNav: (k: string) => void }) {
  ...
  return (
    <div className="space-y-8">
      {/* Page header */}
      <div>
        <h1 className="font-display text-2xl sm:text-3xl text-teal-900">Good morning 👋</h1>
        <p className="text-teal-500 text-sm mt-1">{fmtDate(today)} — Here&apos;s your clinic at a glance</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="p-5">
          ...
          />
          {todayApts.length === 0 ? (
            <div className="text-center py-10 text-teal-300 text-sm">No appointments today</div>
          ) : (
                ...
                return (
                  <div key={apt.id} className="flex items-center gap-3 p-3 rounded-xl bg-teal-50/50 border border-teal-50">
                    <div className="w-12 text-center">
                      <div className="text-xs font-bold text-teal-700">{apt.time}</div>
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="font-medium text-teal-800 text-sm truncate">{patient?.fullName}</div>
                      <div className="text-xs text-teal-500 truncate">{apt.type} · {dentist?.fullName}</div>
                    </div>
        ...
        <Card className="p-5">
          ...
          <div className="space-y-3">
            {MOCK_DENTISTS.map(d => {
              ...
              return (
                <div key={d.id} className="flex items-center gap-3 p-3 rounded-xl border border-teal-50 bg-white">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-teal-800 text-sm truncate">{d.fullName}</div>
                    <div className="text-xs text-teal-400 truncate">{d.specialization}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-bold text-teal-700">{dApts.length}</div>
                    <div className="text-xs text-teal-400">today</div>
                  </div>
                </div>
              );
            })}
        ...
        <Card className="p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {upcomingAdj.map(p => {
              ...
              return (
                <div key={p.id} className={`p-3.5 rounded-xl border ${daysLeft <= 3 ? 'bg-amber-50 border-amber-100' : 'bg-teal-50 border-teal-100'}`}>
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <div className="font-medium text-teal-800 text-sm leading-tight">{p.fullName}</div>
                    {daysLeft <= 3 && <AlertCircle size={14} className="text-amber-500 flex-shrink-0 mt-0.5" />}
                  </div>
                  <div className="text-xs text-teal-500">{fmtShortDate(p.nextAdjustmentDate!)}</div>
                  <div className={`mt-1.5 text-xs font-semibold ${daysLeft <= 3 ? 'text-amber-600' : 'text-teal-600'}`}>
