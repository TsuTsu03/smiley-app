...
export default function AdminOverview({ onNav }: { onNav: (k: string) => void }) {
  ...
  const allApts     = MOCK_APPOINTMENTS.filter(a => a.clinicId === cid);
  const allPatients = MOCK_PATIENTS.filter(p => p.clinicId === cid);
  const allDentists = MOCK_DENTISTS.filter(d => d.clinicId === cid);
  const todayApts   = allApts.filter(a => a.date === today);
  const upcoming    = allApts.filter(a => a.date > today && a.status !== 'cancelled');
  const now = new Date(); const future = new Date(); future.setDate(future.getDate() + 14);
  const upcomingAdj = allPatients.filter(p => {
    if (!p.nextAdjustmentDate) return false;
    const d = new Date(p.nextAdjustmentDate); return d >= now && d <= future;
  });
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard icon={<Users size={20} />}     label="Total Patients"      value={allPatients.length}        sub="Registered" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="p-5">
          ...
          <div className="space-y-3">
            {allDentists.map(d => {
              const dApts = allApts.filter(a => a.dentistId === d.id && a.date === today);
              return (
