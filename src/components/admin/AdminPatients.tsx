      ...
      <Card className="p-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-blue-100 bg-blue-50/30 text-sm text-blue-900 focus:border-blue-400 transition-colors"
          />
        </div>
      </Card>
            ...
            <div className="md:hidden divide-y divide-blue-50">
              {patients.map(p => {
                ...
                const daysLeft = p.nextAdjustmentDate
                  ...
                  : null;
                return (
                  <div key={p.id} className="flex items-center gap-3 px-4 py-3.5 hover:bg-blue-50/30 transition-colors">
                    <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-300 to-blue-500 text-white text-xs font-bold flex items-center justify-center flex-shrink-0">
                      {p.fullName.split(' ').map(n => n[0]).slice(0, 2).join('')}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="font-medium text-blue-800 text-sm truncate">{p.fullName}</div>
                      <div className="text-xs text-blue-500">{p.phone}</div>
                      <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                        <span className="text-xs text-blue-400">{calcAge(p.dateOfBirth)} yrs · {p.gender}</span>
                        <span className="bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded text-xs">{records.length} records</span>
                        {daysLeft !== null && (
                          <span className={`text-xs font-semibold ${daysLeft <= 7 ? 'text-amber-600' : 'text-blue-600'}`}>
                            Adj: {daysLeft}d
                          </span>
                        )}
                      </div>
                ...
                <tbody>
                  {patients.map((p, i) => {
                    ...
                    const daysLeft  = p.nextAdjustmentDate
                      ...
                      : null;
                    return (
                      <tr key={p.id} className={`border-b border-blue-50/80 hover:bg-blue-50/40 transition-colors ${i % 2 ? 'bg-blue-50/20' : ''}`}>
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-blue-300 to-blue-500 text-white text-xs font-bold flex items-center justify-center flex-shrink-0">
                              {p.fullName.split(' ').map(n => n[0]).slice(0, 2).join('')}
                            </div>
                            <div>
                              <div className="font-medium text-blue-800">{p.fullName}</div>
                              <div className="text-xs text-blue-400 capitalize">{p.gender}</div>
                            </div>
                          </div>
                        </td>
                        <td className="px-5 py-3.5">
                          <div className="text-blue-700">{p.phone}</div>
                          <div className="text-xs text-blue-400">{p.email}</div>
                        </td>
                        <td className="px-5 py-3.5">
                          <div className="text-blue-700">{fmtShortDate(p.dateOfBirth)}</div>
                          <div className="text-xs text-blue-400">{calcAge(p.dateOfBirth)} years old</div>
                        </td>
                        <td className="px-5 py-3.5">
                          {p.nextAdjustmentDate ? (
                            <>
                              <div className={`font-medium text-sm ${daysLeft! <= 7 ? 'text-amber-600' : 'text-blue-700'}`}>
                                {fmtShortDate(p.nextAdjustmentDate)}
                              </div>
                              <div className="text-xs text-blue-400">{daysLeft} days away</div>
                            </>
    ...
    <div>
      <div className="flex gap-2 mb-5">
        {(['info', 'records'] as const).map(v => (
          <button key={v} onClick={() => setView(v)}
            className={`px-4 py-1.5 rounded-lg text-sm font-medium capitalize transition-colors ${view === v ? 'bg-blue-600 text-white' : 'bg-blue-50 text-blue-600 hover:bg-blue-100'}`}>
      ...
      {view === 'info' ? (
        <div className="space-y-5">
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              <div key={k}>
                <div className="text-xs text-blue-400 mb-0.5">{k}</div>
                <div className="font-medium text-blue-800 text-sm">{v}</div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-3">
          {records.length === 0 ? (
            <div className="text-center py-10 text-blue-300 text-sm">No records yet</div>
...
function RecordCard({ record }: { record: MedicalRecord }) {
  ...
  return (
    <div className="border border-blue-100 rounded-xl p-4 space-y-2">
      <div className="flex items-start justify-between">
        <div>
          <div className="font-semibold text-blue-800">{record.procedure}</div>
          <div className="text-xs text-blue-400">{fmtDate(record.date)} · {dentist?.fullName}</div>
        </div>
        {record.tooth && <span className="text-xs bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full">{record.tooth}</span>}
      </div>
      <div className="text-sm text-blue-600">{record.diagnosis}</div>
      {record.notes && <div className="text-sm text-blue-500 bg-blue-50 rounded-lg px-3 py-2">{record.notes}</div>}
      {record.prescription && (
