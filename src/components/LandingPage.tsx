...
function AmbientBackground({ tone = "light" }: { tone?: "light" | "dark" }) {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none" aria-hidden>
      ...
      {/* drifting auroras */}
      <motion.div
        animate={{ x: [0, 60, 0], y: [0, -40, 0], scale: [1, 1.15, 1], opacity: [0.3, 0.5, 0.3] }}
        transition={{ duration: 14, repeat: Infinity, ease: "easeInOut" }}
        className={`absolute -top-24 -left-24 w-[34rem] h-[34rem] rounded-full blur-3xl ${dark ? "bg-sky-500/25" : "bg-sky-300/30"}`}
      />
      <motion.div
        animate={{ x: [0, -50, 0], y: [0, 40, 0], scale: [1, 1.2, 1], opacity: [0.25, 0.45, 0.25] }}
...
const DEMO_SCENES = [
  {
    ...
    render: () => (
      <div className="w-full max-w-sm">
        <div className="text-[11px] font-semibold text-sky-900 mb-3">Pick a time with Dr. Santos</div>
  ...
  },
  {
    ...
    render: () => (
      <div className="w-full max-w-sm space-y-2.5">
        {[
          { c: "SMS", t: "Hi Andrea! Your cleaning is tomorrow at 1:15 PM 🦷", d: "Sent · 2s ago", on: true },
          { c: "Email", t: "Appointment confirmation + clinic directions", d: "Delivered", on: true },
          { c: "Follow-up", t: "Scheduled for 6 months from visit", d: "Queued", on: false },
        ].map(({ c, t, d, on }, i) => (
          <motion.div
            key={c}
          ...
          >
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${on ? "bg-amber-50 text-amber-500" : "bg-sky-50 text-sky-400"}`}>
            ...
            <div className="min-w-0">
              ...
              <div className="text-[9px] text-teal-500 font-medium mt-0.5">{d}</div>
            </div>
          </motion.div>
        ))}
      </div>
    ...
    render: () => (
      <div className="w-full max-w-sm">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-purple-400 to-sky-400 text-white flex items-center justify-center text-[11px] font-bold">AC</div>
          <div>
            <div className="text-[12px] font-semibold text-sky-900">Andrea Cruz</div>
            <div className="text-[10px] text-sky-400">Record #2048 · Updated just now</div>
          </div>
        </div>
        <div className="space-y-2">
          {[
            ...
            >
              <div className="text-[9px] uppercase tracking-wide text-sky-400 font-semibold">{l}</div>
...
function DemoPlayer({ large = false }: { large?: boolean }) {
  ...
  useEffect(() => {
    if (!inView) return;
    const id = setInterval(() => setScene((s) => (s + 1) % DEMO_SCENES.length), 3600);
  ...
  return (
    <div ref={containerRef} className="bg-white rounded-2xl border border-sky-100 shadow-2xl shadow-sky-900/15 overflow-hidden">
      {/* player chrome */}
      <div className="flex items-center gap-2 px-4 h-10 border-b border-sky-50 bg-sky-50/50">
        ...
        <div className="ml-2 flex items-center gap-1.5 text-[11px] text-sky-500 font-semibold">
          <span className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse" /> Live product demo
        </div>
      </div>

      {/* stage */}
      <div className={`relative bg-gradient-to-br from-sky-50/60 via-white to-teal-50/40 flex items-center justify-center ${large ? "min-h-[22rem] p-8" : "min-h-[19rem] p-7"}`}>
        {/* current-step badge */}
        <div className="absolute top-4 left-4 flex items-center gap-2">
          <div className={`w-7 h-7 rounded-lg bg-gradient-to-br ${active.tone} flex items-center justify-center text-white shadow-md`}>
            <ActiveIcon size={15} />
          </div>
          ...
          >
            <div className="h-1 rounded-full bg-sky-100 overflow-hidden">
              <motion.div
                ...
                transition={{ duration: i === scene ? 3.4 : 0.3, ease: "linear" }}
                key={`${s.key}-${scene}`}
              />
            </div>
          </button>
...
function AIPreview() {
  ...
  const risks = [
    { i: "PR", name: "Paolo Ramos", time: "3:00 PM", level: "High", pct: 82, tone: "bg-rose-500", chip: "text-rose-600 bg-rose-50" },
    { i: "MV", name: "Mark Villanueva", time: "10:30 AM", level: "Medium", pct: 47, tone: "bg-amber-500", chip: "text-amber-600 bg-amber-50" },
    { i: "LM", name: "Liza Mariano", time: "9:00 AM", level: "Low", pct: 12, tone: "bg-teal-500", chip: "text-teal-600 bg-teal-50" },
  ];

  return (
    <div ref={ref} className="bg-white rounded-2xl border border-sky-100 shadow-2xl shadow-sky-900/20 overflow-hidden">
      {/* header */}
      <div className="flex items-center gap-2.5 px-4 h-12 border-b border-sky-50 bg-gradient-to-r from-sky-50 to-teal-50/60">
        ...
        <div className="flex-1">
          <div className="text-[12px] font-bold text-sky-900 leading-none">Smiley AI</div>
          <div className="text-[9px] text-sky-400 mt-0.5">Predicting today&apos;s no-shows</div>
        </div>
      ...
      <div className="p-4 space-y-2.5 bg-gradient-to-br from-white to-sky-50/40">
        <div className="text-[10px] font-semibold text-sky-500 uppercase tracking-wide">No-show risk · today</div>
        {risks.map((r, idx) => (
          <motion.div
            key={r.name}
          ...
          >
            <div className={`w-7 h-7 rounded-full ${r.tone} flex items-center justify-center text-white text-[9px] font-bold shrink-0`}>{r.i}</div>
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-semibold text-sky-900 truncate">{r.name}</span>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${r.chip}`}>{r.level}</span>
              </div>
            </div>
          </motion.div>
        ))}
      ...
      >
        <div className="flex items-start gap-2">
          ...
          <p className="text-[11px] text-sky-800 leading-snug">
            I drafted reminders for the <span className="font-semibold">2 high-risk</span> patients. Send them now?
          </p>
        </div>
        <div className="flex items-center gap-2 mt-2.5 pl-8">
          ...
          <span className="text-[10px] font-medium text-sky-500 px-2 py-1">Edit</span>
        </div>
      </motion.div>
    </div>
  );
...
const FeaturePreviews = {
  records: (
    <div className="flex items-center gap-2.5 rounded-lg bg-sky-50/70 border border-sky-100 px-2.5 py-2">
      <div className="w-7 h-7 rounded-full bg-gradient-to-br from-sky-400 to-teal-400 flex items-center justify-center text-white text-[9px] font-bold shrink-0">AC</div>
      <div className="min-w-0">
        <div className="text-[10px] font-semibold text-sky-900 truncate">Andrea Cruz · #2048</div>
        <div className="text-[9px] text-sky-400 truncate">Scaling · Mild gingivitis</div>
      </div>
    </div>
  ),
  booking: (
  ...
  reminders: (
    <div className="space-y-1.5">
      <div className="flex items-center gap-2 rounded-lg bg-amber-50/70 border border-amber-100 px-2.5 py-1.5">
        ...
        <span className="text-[10px] text-sky-700 truncate">SMS sent · cleaning tomorrow 1:15</span>
      </div>
      <div className="flex items-center gap-2 rounded-lg bg-sky-50/70 border border-sky-100 px-2.5 py-1.5">
        ...
        <span className="text-[10px] text-sky-700 truncate">Email confirmation delivered</span>
      </div>
    </div>
  ),
  branches: (
    <div className="flex gap-1.5">
      {["BGC", "Makati", "QC"].map((b, i) => (
        <div key={b} className={`flex-1 rounded-md border px-2 py-1.5 text-center ${i === 0 ? "bg-purple-50 border-purple-200" : "bg-white border-sky-100"}`}>
  ...
  dentist: (
    <div className="space-y-1">
      {[{ t: "9:00", n: "Liza M." }, { t: "10:30", n: "Mark V." }].map(({ t, n }) => (
        <div key={t} className="flex items-center gap-2 rounded-md bg-rose-50/60 border border-rose-100 px-2 py-1">
          <span className="text-[9px] font-bold text-rose-500 shrink-0">{t}</span>
...
const FEATURES = [
  { icon: Users, title: "Patient Records", desc: "Complete medical history, treatment plans, and allergies — all in one secure profile.", tone: "from-sky-500 to-sky-600", preview: FeaturePreviews.records },
  { icon: Calendar, title: "Smart Booking", desc: "Let patients choose a dentist, date, and time — without the back-and-forth messages.", tone: "from-teal-500 to-teal-600", preview: FeaturePreviews.booking },
  { icon: Bell, title: "Automated Reminders", desc: "SMS and email reminders for upcoming appointments and follow-up visits.", tone: "from-amber-500 to-amber-600", preview: FeaturePreviews.reminders },
  { icon: Building2, title: "Multi-Clinic Ready", desc: "Manage multiple branches under one account. Each clinic gets its own branded portal.", tone: "from-purple-500 to-purple-600", preview: FeaturePreviews.branches },
  { icon: Stethoscope, title: "Dentist Dashboard", desc: "Dentists view their schedule, patient records, and add treatment notes on the go.", tone: "from-rose-500 to-rose-600", preview: FeaturePreviews.dentist },
...
const PAIN_POINTS = [
  { icon: MessageSquare, text: "Bookings buried in Messenger and text threads" },
...
const BEFORE_AFTER = [
  { before: "Messenger & text bookings", after: "Branded online booking portal" },
  { before: "Paper files & spreadsheets", after: "Organized digital patient records" },
  { before: "Manual chat reminders", after: "Automatic SMS & email reminders" },
  { before: "Frequent missed appointments", after: "Fewer no-shows, full chairs" },
  { before: "No daily overview", after: "Real-time clinic dashboard" },
];

/* ROI / outcome cards — clinic owners buy outcomes, not features */
const BENEFITS = [
  { icon: Bell, title: "Reduce no-shows", desc: "Automated SMS and email reminders before every visit keep chairs full." },
  { icon: Clock, title: "Save admin hours", desc: "Patients book online without the back-and-forth — your front desk gets time back." },
  { icon: SmilePlus, title: "Improve retention", desc: "Follow-up reminders and organized treatment history bring patients back." },
  { icon: TrendingUp, title: "Track performance", desc: "Appointments, revenue, new patients, and attendance in one live dashboard." },
];

/* Role-based value — helps every decision-maker see themselves */
const ROLES = [
  { icon: TrendingUp, title: "Clinic owners", desc: "Track revenue, appointments, patient growth, and branch performance at a glance." },
  { icon: Stethoscope, title: "Dentists", desc: "View schedules, patient records, notes, and full treatment history on any device." },
  { icon: CalendarCheck, title: "Front desk staff", desc: "Manage bookings, reminders, cancellations, and patient queues in seconds." },
  { icon: SmilePlus, title: "Patients", desc: "Book online, view appointment history, and get reminders — no calls needed." },
];

/* Trust & security builders — critical for healthcare buyers */
const TRUST = [
  { icon: Lock, title: "Secure patient records", desc: "Encrypted storage with strict access controls." },
  { icon: UserCog, title: "Role-based access", desc: "Owners, dentists, and staff each see only what they need." },
  { icon: Cloud, title: "Automatic cloud backup", desc: "Your data is backed up and recoverable, always." },
  { icon: Download, title: "Exportable records", desc: "Own your data — export patient records anytime." },
  { icon: ScrollText, title: "Audit logs", desc: "See who accessed or changed records, and when." },
  { icon: ShieldCheck, title: "Privacy-focused design", desc: "Built for the trust that clinic operations demand." },
];

/* Smiley AI capabilities — the modern differentiator */
const AI_FEATURES = [
  { icon: Brain, title: "No-show predictor", desc: "AI scores every appointment by no-show risk so you can send a nudge or fill the slot early." },
  { icon: Mic, title: "AI charting scribe", desc: "Speak naturally after a visit — AI turns it into structured, ready-to-save treatment notes." },
  { icon: Bot, title: "AI front-desk assistant", desc: "A 24/7 chatbot that books, reschedules, and answers patients in Tagalog and English." },
  { icon: BarChart3, title: "Ask-your-data analytics", desc: "Ask in plain words — “Ilang new patients this month?” — and get instant answers and charts." },
];
...
// Trust-row avatar initials shown above the fold
const TRUST_AVATARS = [
  { i: "MS", tone: "from-teal-400 to-sky-400" },
  { i: "JR", tone: "from-sky-400 to-purple-400" },
  { i: "RM", tone: "from-amber-400 to-rose-400" },
  { i: "AC", tone: "from-purple-400 to-sky-400" },
  { i: "PR", tone: "from-rose-400 to-teal-400" },
];

// Faux clinic "logos" for the trust marquee (wordmarks, no assets needed)
const CLINIC_LOGOS = [
  { name: "BrightSmile", icon: SmilePlus },
  { name: "PearlWhite", icon: Sparkles },
  { name: "SmileCare BGC", icon: Stethoscope },
  { name: "DentaPro", icon: ShieldCheck },
  { name: "OraClinic", icon: Building2 },
  { name: "GentleDental", icon: SmilePlus },
];
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-white font-sans overflow-x-hidden">
      ...
      <section className="relative pt-28 pb-16 sm:pt-32 sm:pb-20 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-5 sm:px-8">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-10 items-center">
            ...
            >
              <motion.div variants={fadeUp} className="flex flex-wrap items-center justify-center lg:justify-start gap-2 mb-6">
                ...
                <a
                  ...
                  style={{ background: "linear-gradient(135deg, #0284C7, #14B8A6)" }}
                >
              ...
              >
                Smiley helps dental clinics manage online bookings, patient records, automated
                reminders, dentist schedules, and follow-ups — without spreadsheets or manual
                chat reminders.
              </motion.p>

              <motion.div variants={fadeUp} className="flex flex-wrap items-center justify-center lg:justify-start gap-3.5">
                <Link
                  href="/register"
              ...
              <motion.div
                variants={fadeUp}
                className="flex flex-col sm:flex-row items-center lg:items-start justify-center lg:justify-start gap-4 sm:gap-5 mt-7"
              >
                <div className="flex -space-x-2.5">
                  {TRUST_AVATARS.map(({ i, tone }) => (
                    <div
                      key={i}
                      className={`w-9 h-9 rounded-full bg-gradient-to-br ${tone} ring-2 ring-white flex items-center justify-center text-white text-[10px] font-bold`}
                    >
                      {i}
                    </div>
                  ))}
                ...
                <div className="text-center sm:text-left">
                  <div className="flex items-center justify-center sm:justify-start gap-1">
                    ...
                    <span className="ml-1 text-sm font-bold text-sky-900">4.9/5</span>
                  </div>
                  <div className="text-xs text-sky-500">Loved by <span className="font-semibold text-sky-700">growing dental clinics</span></div>
                </div>
              </motion.div>
            </motion.div>
              ...
              <div className="relative">
                ...
                >
                  <span className="flex items-center gap-2.5 px-5 py-3 rounded-full bg-white/90 backdrop-blur shadow-2xl shadow-sky-900/20 border border-white/60 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <span className="text-sm font-semibold text-sky-900">Play demo</span>
                  </span>
                </button>
                ...
                >
                  ...
                  <div>
                    <div className="text-[11px] font-semibold text-sky-900">Reminder sent</div>
                    <div className="text-[10px] text-sky-400">SMS · Andrea C.</div>
                  </div>
                </motion.div>
      ...
      {/* ═══ LOGO MARQUEE ═══════════════════════════════════════ */}
      <section className="py-10 border-y border-sky-100/70 bg-white overflow-hidden">
        <p className="text-center text-xs font-semibold uppercase tracking-wider text-sky-400 mb-6">
          Trusted by dental clinics across the country
        </p>
        <div className="relative">
          {/* edge fades */}
          ...
          >
            {[...CLINIC_LOGOS, ...CLINIC_LOGOS].map(({ name, icon: Icon }, i) => (
              <div key={i} className="flex items-center gap-2.5 text-sky-300 shrink-0">
                <Icon size={22} />
                <span className="font-display text-xl font-bold text-sky-400/80">{name}</span>
              </div>
            ))}
          </motion.div>
        </div>
      ...
      <section className="relative overflow-hidden py-20 sm:py-28">
        ...
        <div className="relative max-w-5xl mx-auto px-5 sm:px-8">
          ...
          >
            <motion.div variants={fadeUp} className="inline-flex items-center gap-2 bg-white/10 backdrop-blur border border-white/15 px-3.5 py-1 rounded-full text-rose-200 text-xs font-semibold mb-5">
              <span className="w-1.5 h-1.5 rounded-full bg-rose-400 animate-pulse" /> Sound familiar?
            </motion.div>
            <motion.h2 variants={fadeUp} className="font-display text-3xl sm:text-5xl text-white mb-4 tracking-tight leading-tight">
              Still running your clinic through Messenger, paper records, and spreadsheets?
            </motion.h2>
            <motion.p variants={fadeUp} className="text-sky-200/70 text-lg">
              Every missed message is a missed appointment. Here&apos;s what&apos;s quietly costing
              your clinic time and patients.
            </motion.p>
          </motion.div>
          ...
          >
            {PAIN_POINTS.map(({ icon: Icon, text }) => (
              ...
              >
                ...
                <p className="text-sky-100/90 text-sm font-medium leading-snug pt-1.5">{text}</p>
              </motion.div>
            ))}
            <motion.div
              variants={fadeUp}
            ...
            >
              <p className="text-white font-semibold text-sm mb-1">There&apos;s a better way.</p>
              <p className="text-sky-200/80 text-sm leading-snug">Smiley replaces all of it with one simple platform built for dental clinics.</p>
            </motion.div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-20 sm:py-28 bg-gradient-to-b from-white to-sky-50/50">
        <div className="max-w-5xl mx-auto px-5 sm:px-8">
          ...
          >
            {/* Before */}
            <motion.div variants={fadeUp} className="rounded-2xl border border-sky-100 bg-white p-6 sm:p-7 shadow-soft">
              <div className="flex items-center gap-2.5 mb-5">
                <div className="w-9 h-9 rounded-xl bg-rose-50 flex items-center justify-center">
                  <XCircle size={18} className="text-rose-400" />
                </div>
                <span className="font-display text-xl text-sky-950">Before Smiley</span>
              </div>
              <ul className="space-y-3.5">
                {BEFORE_AFTER.map(({ before }) => (
                  <li key={before} className="flex items-center gap-3 text-sky-500 text-sm">
                    <XCircle size={16} className="text-rose-300 shrink-0" />
                    <span className="line-through decoration-rose-200">{before}</span>
                  </li>
                ))}
              </ul>
            </motion.div>
            ...
            {/* After */}
            <motion.div variants={fadeUp} className="relative rounded-2xl p-6 sm:p-7 shadow-card bg-gradient-to-br from-sky-50 to-teal-50/60 border border-teal-200/50">
              <div className="flex items-center gap-2.5 mb-5">
                ...
                <span className="font-display text-xl text-sky-950">After Smiley</span>
              </div>
              <ul className="space-y-3.5">
                {BEFORE_AFTER.map(({ after }) => (
                  <li key={after} className="flex items-center gap-3 text-sky-800 text-sm font-medium">
          ...
          >
            {BENEFITS.map(({ icon: Icon, title, desc }) => (
              ...
              >
                ...
                <h3 className="font-semibold text-sky-900 text-lg mb-1.5">{title}</h3>
                <p className="text-sky-600/70 text-sm leading-relaxed">{desc}</p>
              </motion.div>
            ))}
          </motion.div>
        </div>
      ...
      {/* ═══ STATS ══════════════════════════════════════════════ */}
      <section className="bg-gradient-to-r from-sky-50/70 via-white to-teal-50/50 border-b border-sky-100/70">
      ...
      <section className="py-20 sm:py-28 relative overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-5 sm:px-8 grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <motion.div
            ...
            variants={stagger}
          >
            <motion.div variants={fadeUp} className="inline-flex items-center gap-2 bg-rose-50 border border-rose-200/60 px-3.5 py-1 rounded-full text-rose-600 text-xs font-semibold mb-5">
              <span className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse" /> See it in action
            </motion.div>
            <motion.h2 variants={fadeUp} className="font-display text-3xl sm:text-5xl text-sky-950 mb-5 tracking-tight leading-tight">
      ...
      {/* ═══ FEATURES (bento) ═══════════════════════════════════ */}
      <section id="features" className="py-20 sm:py-28 scroll-mt-20 relative bg-gradient-to-b from-white to-sky-50/40">
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <motion.div
            initial="hidden"
            whileInView="visible"
          ...
          >
            {FEATURES.map(({ icon: Icon, title, desc, tone, preview }) => (
              ...
              >
                ...
                <div className={`absolute inset-0 bg-gradient-to-br ${tone} opacity-0 group-hover:opacity-[0.05] transition-opacity`} />
                {/* corner glow */}
                <div className={`absolute -top-10 -right-10 w-28 h-28 bg-gradient-to-br ${tone} opacity-0 group-hover:opacity-20 blur-2xl transition-opacity rounded-full`} />
                <div className={`relative w-12 h-12 rounded-xl bg-gradient-to-br ${tone} flex items-center justify-center mb-5 shadow-lg shadow-sky-500/10 group-hover:scale-110 group-hover:-rotate-3 transition-transform`}>
                  <Icon size={22} className="text-white" />
                </div>
                <h3 className="relative font-semibold text-sky-900 text-lg mb-2">{title}</h3>
                <p className="relative text-sky-600/70 text-sm leading-relaxed mb-5">{desc}</p>
      ...
      {/* ═══ SMILEY AI (dark) ═══════════════════════════════════ */}
      <section id="ai" className="relative overflow-hidden py-20 sm:py-28 scroll-mt-20">
        <AmbientBackground tone="dark" />
        <div className="relative max-w-6xl mx-auto px-5 sm:px-8">
          ...
          >
            <motion.div variants={fadeUp} className="inline-flex items-center gap-2 bg-white/10 backdrop-blur border border-white/15 px-4 py-1.5 rounded-full text-white text-sm font-medium mb-6">
              <Sparkles size={14} className="text-teal-300" /> Powered by Smiley AI
            </motion.div>
            <motion.h2 variants={fadeUp} className="font-display text-3xl sm:text-5xl text-white mb-4 tracking-tight leading-tight">
              Your clinic&apos;s new AI teammate
            </motion.h2>
            <motion.p variants={fadeUp} className="text-sky-200/75 text-lg">
              Smiley AI quietly handles the busywork — predicting no-shows, writing up visits, and
              answering patients — so your team can focus on care.
            </motion.p>
          </motion.div>
            ...
            >
              {AI_FEATURES.map(({ icon: Icon, title, desc }) => (
                ...
                >
                  ...
                  <p className="text-sky-200/70 text-sm leading-relaxed">{desc}</p>
                </motion.div>
              ))}
            </motion.div>
          </div>
          ...
          >
            Smiley AI assists your team — every suggestion stays under clinic review and control.
          </motion.p>
        </div>
      </section>
      ...
      <section className="py-20 sm:py-28 bg-gradient-to-b from-sky-50/40 to-white">
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          ...
          >
            ...
            <motion.p variants={fadeUp} className="text-sky-700/60 text-lg">
              From the owner&apos;s office to the front desk to the patient&apos;s phone — each role gets exactly what they need.
            </motion.p>
          </motion.div>
            ...
            {ROLES.map(({ icon: Icon, title, desc }) => (
              ...
              >
                ...
                <div className="w-11 h-11 rounded-xl bg-sky-50 border border-sky-100 flex items-center justify-center mb-4 group-hover:bg-gradient-to-br group-hover:from-sky-500 group-hover:to-teal-500 transition-colors">
                  <Icon size={20} className="text-sky-600 group-hover:text-white transition-colors" />
                </div>
                <div className="text-[10px] uppercase tracking-wider text-teal-600 font-semibold mb-1">For</div>
      ...
      <section className="py-20 sm:py-28 bg-gradient-to-b from-white to-sky-50/50">
        <div className="max-w-6xl mx-auto px-5 sm:px-8">
          ...
          >
            ...
            <motion.p variants={fadeUp} className="text-sky-700/60 text-lg">
              Healthcare runs on trust. Smiley is built to keep patient records secure, private, and always recoverable.
            </motion.p>
          </motion.div>
          ...
          >
            {TRUST.map(({ icon: Icon, title, desc }) => (
              ...
              >
                ...
                <div>
                  <h3 className="font-semibold text-sky-900 text-[15px] mb-0.5">{title}</h3>
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          ...
          >
            {TESTIMONIALS.map(({ name, role, quote }) => (
              ...
              >
                <MessageSquare size={28} className="text-sky-100 absolute top-6 right-6" />
        ...
        <div className="max-w-5xl mx-auto px-5 sm:px-8">
          ...
          >
            ...
            <motion.div variants={fadeUp} className="relative inline-flex items-center gap-2 bg-white/10 backdrop-blur border border-white/20 px-4 py-1.5 rounded-full text-white text-sm font-medium mb-6">
              <Sparkles size={14} className="text-teal-300" /> Start managing your clinic today
            </motion.div>
            ...
            <motion.p variants={fadeUp} className="relative text-sky-200/80 text-lg mb-9 max-w-xl mx-auto">
              Launch your booking portal, automate reminders, and organize patient records in one place.
              Setup takes less than 10 minutes — no credit card needed.
            </motion.p>
