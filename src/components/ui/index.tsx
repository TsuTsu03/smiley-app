...
export function Card({ children, className = '', onClick }: { children: ReactNode; className?: string; onClick?: () => void }) {
  return (
    <div
      onClick={onClick}
      className={clsx(
        'bg-white rounded-2xl border border-sky-50 shadow-soft',
  ...
  return (
    <Card className="p-5">
      <div className="flex items-start justify-between">
        <div>
          <div className="text-xs font-medium text-sky-500 uppercase tracking-wide mb-1">{label}</div>
          <div className="text-2xl font-bold text-sky-900">{value}</div>
          {sub && <div className="text-xs text-sky-500 mt-0.5">{sub}</div>}
        </div>
  ...
  return (
    <div className="flex flex-wrap items-start justify-between gap-2 mb-5">
      <div className="min-w-0">
        <h2 className="font-display text-lg sm:text-xl text-sky-900 leading-snug">{title}</h2>
        {sub && <p className="text-sm text-sky-500 mt-0.5">{sub}</p>}
      </div>
...
export function EmptyState({ icon, title, desc }: { icon: ReactNode; title: string; desc: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      <div className="w-16 h-16 rounded-2xl bg-sky-50 text-sky-300 flex items-center justify-center mb-4">{icon}</div>
      <div className="font-semibold text-sky-700 mb-1">{title}</div>
      <div className="text-sm text-sky-400 max-w-xs">{desc}</div>
    </div>
  );
}
...
export function Divider() {
  return <div className="border-t border-sky-50 my-5" />;
}
...
export function Modal({ onClose, title, children, wide = false }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-sky-950/25 backdrop-blur-sm p-0 sm:p-4">
      <div className={clsx('bg-white w-full overflow-hidden animate-slide-up rounded-t-3xl sm:rounded-3xl shadow-hover', wide ? 'sm:max-w-2xl' : 'sm:max-w-lg')}>
        <div className="flex items-center justify-between px-4 sm:px-7 py-4 sm:py-5 border-b border-sky-50">
          <h3 className="font-display text-lg sm:text-xl text-sky-900">{title}</h3>
          <button onClick={onClose} className="p-1.5 text-sky-400 hover:text-sky-600 hover:bg-sky-50 rounded-lg transition-colors">
            ✕
          </button>
        </div>
...
export function Input({ label, ...props }: { label: string } & React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <div>
      <label className="block text-sm font-medium text-sky-800 mb-1.5">{label}</label>
      <input
        {...props}
        className="w-full px-4 py-2.5 rounded-xl border border-sky-100 bg-sky-50/30 text-sky-900 placeholder-sky-300 focus:border-sky-400 focus:bg-white transition-colors text-sm"
      />
    </div>
  );
}
...
export function Select({ label, children, ...props }: { label: string } & React.SelectHTMLAttributes<HTMLSelectElement>) {
  return (
    <div>
      ...
      <select
        {...props}
        className="w-full px-4 py-2.5 rounded-xl border border-sky-100 bg-sky-50/30 text-sky-900 focus:border-sky-400 focus:bg-white transition-colors text-sm"
      >
        {children}
      </select>
    </div>
...
export function Textarea({ label, ...props }: { label: string } & React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return (
    <div>
      ...
      <textarea
        {...props}
        className="w-full px-4 py-2.5 rounded-xl border border-sky-100 bg-sky-50/30 text-sky-900 placeholder-sky-300 focus:border-sky-400 focus:bg-white transition-colors text-sm resize-none"
      />
    </div>
  );
}
...
export function Btn({ children, onClick, variant = 'primary', size = 'md', disabled, type, className = '' }: {
  ...
  const variants = {
    primary:   'bg-gradient-to-r from-sky-700 to-sky-600 text-white shadow-soft hover:opacity-90',
